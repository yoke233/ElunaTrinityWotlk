...
{
#ifdef ELUNA
    sEluna("OnOpenStateChange")->OnOpenStateChange(open);
#endif
...
{
#ifdef ELUNA
    sEluna("OnConfigLoad")->OnConfigLoad(reload);
#endif
...
{
#ifdef ELUNA
    sEluna("OnShutdownInitiate")->OnShutdownInitiate(code, mask);
#endif
...
{
#ifdef ELUNA
    sEluna("OnShutdownCancel")->OnShutdownCancel();
#endif
...
{
#ifdef ELUNA
    sEluna("OnWorldUpdate")->OnWorldUpdate(diff);
#endif
...
#ifdef ELUNA
    sEluna("OnMapChanged")->OnMapChanged(player);
    sEluna("OnPlayerEnter_map")->OnPlayerEnter(map, player);
#endif
...
bool ScriptMgr::OnDummyEffect(Unit* caster, uint32 spellId, SpellEffIndex effIndex, Item* target)
{
    ...
    ASSERT(target);
#ifdef ELUNA
    if (sEluna("OnDummyEffect_item")->OnDummyEffect(caster, spellId, effIndex, target))
        return false;
#endif
...
bool ScriptMgr::OnQuestAccept(Player* player, Item* item, Quest const* quest)
{
    ...
    ASSERT(quest);
#ifdef ELUNA
    if (sEluna("OnQuestAccept_item")->OnQuestAccept(player, item, quest))
        return false;
#endif
...
bool ScriptMgr::OnItemUse(Player* player, Item* item, SpellCastTargets const& targets)
{
    ...
    ASSERT(item);
#ifdef ELUNA
    if (!sEluna("OnItemUse")->OnUse(player, item, targets))
        return true;
#endif
...
bool ScriptMgr::OnItemExpire(Player* player, ItemTemplate const* proto)
{
    ...
    ASSERT(proto);
#ifdef ELUNA
    if (sEluna("OnItemExpire")->OnExpire(player, proto))
        return false;
#endif
...
bool ScriptMgr::OnItemRemove(Player* player, Item* item)
{
    ...
    if (Map* map = player->FindMap())
        if (map->GetEluna()->OnRemove(player, item))
            return false;
#endif
...
bool ScriptMgr::OnDummyEffect(Unit* caster, uint32 spellId, SpellEffIndex effIndex, Creature* target)
{
    ...
    if (Map* map = caster->FindMap())
        if (map->GetEluna()->OnDummyEffect(caster, spellId, effIndex, target))
            return false;
#endif
...
bool ScriptMgr::OnGossipHello(Player* player, Creature* creature)
{
    ...
    ASSERT(creature);
#ifdef ELUNA
    if (sEluna("OnGossipHello_creature")->OnGossipHello(player, creature))
        return true;
#endif
...
bool ScriptMgr::OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    ASSERT(creature);
#ifdef ELUNA
    if (sEluna("OnGossipSelect_creature")->OnGossipSelect(player, creature, sender, action))
        return true;
#endif
...
bool ScriptMgr::OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code)
{
    ...
    ASSERT(code);
#ifdef ELUNA
    if (sEluna("OnGossipSelectCode_creature")->OnGossipSelectCode(player, creature, sender, action, code))
        return true;
#endif
...
bool ScriptMgr::OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
{
    ...
    ASSERT(quest);
#ifdef ELUNA
    if (sEluna("OnQuestAccept")->OnQuestAccept(player, creature, quest))
    {
        player->PlayerTalkClass->ClearMenus();
        return false;
    }
...
bool ScriptMgr::OnQuestReward(Player* player, Creature* creature, Quest const* quest, uint32 opt)
{
    ...
    ASSERT(quest);
#ifdef ELUNA
    if (sEluna("OnQuestReward")->OnQuestReward(player, creature, quest, opt))
    {
        player->PlayerTalkClass->ClearMenus();
        return false;
    }
...
uint32 ScriptMgr::GetDialogStatus(Player* player, Creature* creature)
{
    ...
    if (Map* map = player->FindMap())
    {
        if (uint32 dialogid = map->GetEluna()->GetDialogStatus(player, creature))
        {
            player->PlayerTalkClass->ClearMenus();
            return dialogid;
        }
...
CreatureAI* ScriptMgr::GetCreatureAI(Creature* creature)
{
    ...
    if (Map* map = creature->FindMap())
        if (CreatureAI* luaAI = map->GetEluna()->GetAI(creature))
            return luaAI;
#endif
...
bool ScriptMgr::OnGossipHello(Player* player, GameObject* go)
{
    ...
    if (Map* map = player->FindMap())
    {
        if (map->GetEluna()->OnGossipHello(player, go))
            return true;
        if (map->GetEluna()->OnGameObjectUse(player, go))
            return true;
    }
#endif
...
bool ScriptMgr::OnGossipSelect(Player* player, GameObject* go, uint32 sender, uint32 action)
{
    ...
    ASSERT(go);
#ifdef ELUNA
    if (sEluna("OnGossipSelect")->OnGossipSelect(player, go, sender, action))
        return true;
#endif
...
bool ScriptMgr::OnGossipSelectCode(Player* player, GameObject* go, uint32 sender, uint32 action, const char* code)
{
    ...
    ASSERT(code);
#ifdef ELUNA
    if (sEluna("OnGossipSelectCode")->OnGossipSelectCode(player, go, sender, action, code))
        return true;
#endif
...
bool ScriptMgr::OnQuestAccept(Player* player, GameObject* go, Quest const* quest)
{
    ...
    ASSERT(quest);
#ifdef ELUNA
    if (sEluna("OnQuestAccept")->OnQuestAccept(player, go, quest))
        return false;
#endif
...
bool ScriptMgr::OnQuestReward(Player* player, GameObject* go, Quest const* quest, uint32 opt)
{
    ...
    ASSERT(quest);
#ifdef ELUNA
    if (sEluna("OnQuestReward")->OnQuestReward(player, go, quest, opt))
        return false;
#endif
...
void ScriptMgr::OnGameObjectDestroyed(GameObject* go, Player* player)
{
    ASSERT(go);
#ifdef ELUNA
    ElunaDo(go)->OnDestroyed(go, player);
#endif
...
void ScriptMgr::OnGameObjectDamaged(GameObject* go, Player* player)
{
    ASSERT(go);
#ifdef ELUNA
    ElunaDo(go)->OnDamaged(go, player);
#endif
...
void ScriptMgr::OnGameObjectLootStateChanged(GameObject* go, uint32 state, Unit* unit)
{
    ASSERT(go);
#ifdef ELUNA
    ElunaDo(go)->OnLootStateChanged(go, state);
#endif
...
void ScriptMgr::OnGameObjectStateChanged(GameObject* go, uint32 state)
{
    ASSERT(go);
#ifdef ELUNA
    ElunaDo(go)->OnGameObjectStateChanged(go, state);
#endif
...
bool ScriptMgr::OnAreaTrigger(Player* player, AreaTriggerEntry const* trigger)
{
    ...
    ASSERT(trigger);
#ifdef ELUNA
    if (sEluna("OnAreaTrigger")->OnAreaTrigger(player, trigger))
        return false;
#endif
...
void ScriptMgr::OnWeatherChange(Weather* weather, WeatherState state, float grade)
{
    ...
    /*
#ifdef ELUNA
    ElunaDo(this)->OnChange(weather, weather->GetZone(), state, grade);
#endif
    */
...
void ScriptMgr::OnAuctionRemove(AuctionHouseObject* ah, AuctionEntry* entry)
{
    ...
    ASSERT(entry);
#ifdef ELUNA
    sEluna("OnRemove")->OnRemove(ah, entry);
#endif
...
void ScriptMgr::OnAuctionSuccessful(AuctionHouseObject* ah, AuctionEntry* entry)
{
    ...
    ASSERT(entry);
#ifdef ELUNA
    sEluna("OnSuccessful")->OnSuccessful(ah, entry);
#endif
...
void ScriptMgr::OnAuctionExpire(AuctionHouseObject* ah, AuctionEntry* entry)
{
    ...
    ASSERT(entry);
#ifdef ELUNA
    sEluna("OnExpire")->OnExpire(ah, entry);
#endif
...
{
#ifdef ELUNA
    ElunaDo(killer)->OnCreatureKill(killer, killed);
#endif
...
{
#ifdef ELUNA
    ElunaDo(killer)->OnPlayerKilledByCreature(killer, killed);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnLevelChanged(player, oldLevel);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnFreeTalentPointsChanged(player, points);
#endif
...
{
#ifdef ELUNA
    sEluna("OnTalentsReset")->OnTalentsReset(player, noCost);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnMoneyChanged(player, amount);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnGiveXP(player, amount, victim);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnReputationChange(player, factionID, standing, incremental);
#endif
...
{
#ifdef ELUNA
    ElunaDo(target)->OnDuelRequest(target, challenger);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player1)->OnDuelStart(player1, player2);
#endif
...
{
#ifdef ELUNA
    ElunaDo(winner)->OnDuelEnd(winner, loser, type);
#endif
...
{
#ifdef ELUNA
    sEluna("OnEmote")->OnEmote(player, emote);
#endif
...
{
#ifdef ELUNA
    sEluna("OnTextEmote")->OnTextEmote(player, textEmote, emoteNum, guid);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnSpellCast(player, spell, skipCheck);
#endif
...
#ifdef ELUNA
    if (firstLogin)
        sEluna("OnFirstLogin")->OnFirstLogin(player);
    sEluna("OnLogin")->OnLogin(player);
#endif
...
{
#ifdef ELUNA
    sEluna("OnLogout")->OnLogout(player);
#endif
...
{
#ifdef ELUNA
    sEluna("OnCreate")->OnCreate(player);
#endif
...
{
#ifdef ELUNA
    sEluna("OnDelete")->OnDelete(GUID_LOPART(guid));
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnBindToInstance(player, difficulty, mapid, permanent);
#endif
...
{
#ifdef ELUNA
    ElunaDo(player)->OnUpdateZone(player, newZone, newArea);
#endif
...
{
#ifdef ELUNA
    sEluna("OnAddMember")->OnAddMember(guild, player, plRank);
#endif
...
{
#ifdef ELUNA
    sEluna("OnRemoveMember")->OnRemoveMember(guild, player, isDisbanding);
#endif
...
{
#ifdef ELUNA
    sEluna("OnMOTDChanged")->OnMOTDChanged(guild, newMotd);
#endif
...
{
#ifdef ELUNA
    sEluna("OnInfoChanged")->OnInfoChanged(guild, newInfo);
#endif
...
{
#ifdef ELUNA
    sEluna("OnCreate")->OnCreate(guild, leader, name);
#endif
...
{
#ifdef ELUNA
    sEluna("OnDisband")->OnDisband(guild);
#endif
...
{
#ifdef ELUNA
    sEluna("OnMemberWitdrawMoney")->OnMemberWitdrawMoney(guild, player, amount, isRepair);
#endif
...
{
#ifdef ELUNA
    sEluna("OnMemberDepositMoney")->OnMemberDepositMoney(guild, player, amount);
#endif
...
{
#ifdef ELUNA
    sEluna("OnItemMove")->OnItemMove(guild, player, pItem, isSrcBank, srcContainer, srcSlotId, isDestBank, destContainer, destSlotId);
#endif
...
{
#ifdef ELUNA
    sEluna("OnEvent")->OnEvent(guild, eventType, playerGuid1, playerGuid2, newRank);
#endif
...
{
#ifdef ELUNA
    sEluna("OnBankEvent")->OnBankEvent(guild, eventType, tabId, playerGuid, itemOrMoney, itemStackCount, destTabId);
#endif
...
void ScriptMgr::OnGroupAddMember(Group* group, ObjectGuid guid)
{
    ASSERT(group);
#ifdef ELUNA
    sEluna("OnAddMember")->OnAddMember(group, guid);
#endif
...
void ScriptMgr::OnGroupInviteMember(Group* group, ObjectGuid guid)
{
    ASSERT(group);
#ifdef ELUNA
    sEluna("OnInviteMember")->OnInviteMember(group, guid);
#endif
...
void ScriptMgr::OnGroupRemoveMember(Group* group, ObjectGuid guid, RemoveMethod method, ObjectGuid kicker, const char* reason)
{
    ASSERT(group);
#ifdef ELUNA
    sEluna("OnRemoveMember")->OnRemoveMember(group, guid, method);
#endif
...
void ScriptMgr::OnGroupChangeLeader(Group* group, ObjectGuid newLeaderGuid, ObjectGuid oldLeaderGuid)
{
    ASSERT(group);
#ifdef ELUNA
    sEluna("OnChangeLeader")->OnChangeLeader(group, newLeaderGuid, oldLeaderGuid);
#endif
...
void ScriptMgr::OnGroupDisband(Group* group)
{
    ASSERT(group);
#ifdef ELUNA
    sEluna("OnDisband")->OnDisband(group);
#endif
